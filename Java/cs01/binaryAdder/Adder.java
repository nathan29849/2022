import static e7d849a05f7ba1ad554a5e92e5e2e6aa.Mission1.*;
...
//        boolean newCarry = carry(tempArr[1], carry); // 반쪽 덧셈 결과 중 덧셈 결과를 새로운 올림수(carry)와 carry: 올림 수 확인
        boolean[] carryArr = halfadder(tempArr[1], carry); // carry와 첫 번째 halfadder에서 나온 합을 계산 = [새로운 올림수, 합]
        boolean finalCarry = sum(tempArr[0], carryArr[0]); // 새로운 올림수와 반쪽 덧셈 결과 중 올림 수 결과를 sum: 최종 올림 수 확인
        boolean[] answer = {finalCarry, carryArr[1]};
        return answer;
    }
    ...
    public boolean[] byteadder(boolean[] byteA, boolean[] byteB) {
        boolean carry = false;
        boolean[] tempArr;
        int maxLength = max(byteA.length, byteB.length);
        int minLength = min(byteA.length, byteB.length);
